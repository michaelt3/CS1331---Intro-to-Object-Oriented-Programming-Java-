...
public class GameController {
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            alert.setTitle("Information Dialog");
            alert.setHeaderText(null);
            alert.setContentText("Cannot attack anymore.");
            alert.showAndWait();
            state = GameState.NEUTRAL;
            return;
        }
    ...
    public static void ai() {
        ...
        if (banditHideout != null) {
            if (rand.nextInt(100) < GridFX.getRow()) {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < GridFX.getRow()
                && newCol >= 0 && newCol < GridFX.getRow()) {
