...
public class Map {
    ...
    private void populateMap() {
        TileType[][] tiles = new TileType[GridFX.getRow()][GridFX.getCol()];
        //populates with whatever terrain you want
        //this could be made better if you make map give you an array of
        //TileTypes instead
        Random rand = new Random();
        for (int r = 0; r < GridFX.getRow(); r++) {
            TileType[] row = new TileType[GridFX.getRow()];
            for (int c = 0; c < GridFX.getCol(); c++) {
    ...
    public static void addEnemies(Bandit bandit, int numEnemies) {
        for (int i = 0; i < numEnemies; i++) {
            int r = new Random().nextInt(GridFX.getRow());
            int c = new Random().nextInt(GridFX.getRow());
            putSettlement("Bandit Hideout", bandit, GridFX.getRow() - 1, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(GridFX.getRow());
                c = new Random().nextInt(GridFX.getRow());
            }
            ...
            if (!current.isEmpty()
                ...
                for (int dr = current.getRow() - 1; dr <= current.getRow() + 1;
                    ...
                    for (int dc = current.getCol() - 1;
                        ...
                        if  (dr >= 0 && dr < GridFX.getRow()
                            && dc >= 0 && dc < GridFX.getCol()) {
